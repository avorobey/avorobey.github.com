...
$$F_p(t) \to F_p(t)^{sep} \to \tilde{F_p(t)}$$. Here $$F_p(t)^setp$$ is the separable closure
of $$F_p(t)$$, and it is not equal to the algebraic closure $$\tilde{F_p(t)}$$. To see this, consider
the element $$t^{1/p} \in \tilde{F_p(t)}$$; its minimal polynomial is $$x^p-t$$ which factors as
