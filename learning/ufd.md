...
So now let $$f$$ be such a polynomial, and let $$b(x) = b_nx^n+...+b_0$$ be an irreducible divisor of $$f$$ of the smallest degree possible, which means that $$b(x)$$ is still non-constant. Let $$f(x) = f_1*...f_k$$ be any factorisation, and all $$f_i$$ must also
be non-constant polynomials, of degree $$m_i$$ and with leading coefficients $$c_i$$. We can assume the proof finished for all polynomials of lower degrees than $$f$$. If we find that $$b(x) | f_i$$ for some $$i$$, we're done. Actually, even if we show $$b(x) | c*f_i(x)$$ for some $$c\in R$$, that is also enough, because, given $$b(x)*c'(x) = f_i(x)*c$$, we can factor $$c'(x)$$ and use the argument above to discard the "$$R$$ parts" of both sides; after that the right side will be just $$f_i(x)$$ and the left will still have $$b(x)$$, so $$b(x) | f(x)$$ will be true, and being both irreducible they must be associates.
...
we must end up with an $$f$$ that has no non-trivial $$R$$ part in any of its factorisations,
and if we show uniqueness for such an $$f$$, 
We first prove a small lemma that helps us match the $$R$$ parts of
factorisations with each other.
